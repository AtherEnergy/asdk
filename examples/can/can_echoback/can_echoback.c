...
asdk_can_message_t rxd_message = {
    .message = rx_buffer,
};
...
static void user_can_callback(uint8_t can_ch, asdk_can_event_t event, asdk_can_message_t *can_message)
{
    ...
    case ASDK_CAN_RX_EVENT:
        can_send = true;
        rxd_message.can_id = can_message->can_id;
        rxd_message.dlc = can_message->dlc;
        memcpy(rxd_message.message, can_message->message, can_message->dlc);
        break;
...
int main()
{
    ...
    for (;;)
    {
        ...
        if(can_send == true)
        {
            ...
            if(!tx_status_busy)
            {
                rxd_message.can_id += 1;
                asdk_can_write(ACX_CAN_CH, 0, &rxd_message);
                can_send = false;
            }
        }
