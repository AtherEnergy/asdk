...
void timer1_compare_callback(asdk_timer_event_t timer_event)
{
    ...
    {
        asdk_gpio_output_toggle(user_led_1.mcu_pin);    // led 1 should toggle every 1ms
    }
    else if(timer_event == ASDK_TIMER_MATCH_EVENT)
    {
        asdk_gpio_output_toggle(user_led_4.mcu_pin);    // led 4 should toggle after 100us of led 1
    }
}
